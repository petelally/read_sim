...
% sim_name = "./sims/bssfp_hiperformance";
% sim_name = "./sims/bssfp_default";
% sim_name = "./sims/flash_sims";
sim_name = "./sims/cones_2";
% sim_name = "./sims/cones_2p5";
% sim_name = "./sims/cones_minTR_2";
% sim_name = "./sims/cones_minTR_2p5";
% sim_name = "./sims/cones_minTR_minTE_2";
% sim_name = "./sims/cones_minTR_minTE_2p5";
% sim_name = "./sims/cones_minTR_minTE_5spoil_2";
% sim_name = "./sims/cones_minTR_minTE_5spoil_2p5";
cd(sim_name);

%% Simulation settings
tw_len_ms   = 200;          %duration of sliding time window (ms)
update_rate = 10000;        %number of points to skip in visualiser frames
...
X_mat = readmatrix(dir("./*_GRX.dsv").name, "FileType", "text");
...
while midx<(size(X_mat,1))
    midx=midx+1;
    gidx=gidx+1;
    XG(gidx)=X_mat(midx);
    if X_mat(midx)==X_mat(midx-1)
        XG(gidx+1:gidx+X_mat(midx+1)+1)=XG(gidx);
        gidx=gidx+X_mat(midx+1)+1;
        XG(gidx)=X_mat(midx+2);
        midx=midx+2;
    end
end
...
vert_scl = str2double(vf_txt{2});
Y_mat = readmatrix(dir("./*_GRY.dsv").name, "FileType", "text");
...
while midx<(size(Y_mat,1)-1)
    ...
    if Y_mat(midx)==Y_mat(midx-1)
        YG(gidx+1:gidx+Y_mat(midx+1)+1)=YG(gidx);
        gidx=gidx+Y_mat(midx+1)+1;
        YG(gidx)=Y_mat(midx+2);
        midx=midx+2;
    end
end
...
vert_scl = str2double(vf_txt{2});
Z_mat = readmatrix(dir("./*_GRZ.dsv").name, "FileType", "text");
...
while midx<(size(Z_mat,1)-1)
    ...
    if Z_mat(midx)==Z_mat(midx-1)
        ZG(gidx+1:gidx+Z_mat(midx+1)+1)=ZG(gidx);
        gidx=gidx+Z_mat(midx+1)+1;
        ZG(gidx)=Z_mat(midx+2);
        midx=midx+2;
    end
end
...
for tw_no=1:update_rate:length(XG)-tw_len_us
    ...
    plot((0:(tw_len_us-1))/TS/tw_len_us,abs(fft(XG_int*corrGA)./tw_len_us),"-k")
